      ...
      <CardContent className="flex flex-col gap-4 px-5 pt-5 pb-5">
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-between">
            <Heading as="h3" size="h5" className="text-foreground group-hover:text-primary transition-colors">
              {project.name}
            </Heading>
          </div>
        ...
        <div className="flex flex-col gap-2 rounded-xl bg-secondary p-3 border border-border">
          <div className="flex items-baseline justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              Handover: <strong className="text-foreground">{project.handover}</strong>
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-1.5 pt-1.5 border-t border-primary/10">
            {project.milestones.slice(0, 2).map((m) => (
              <div key={m.label} className="flex items-center justify-between text-[11px]">
                ...
                <span className={cn("font-medium", m.completed ? "text-primary" : "text-foreground/70")}>
                  {m.percent}%
                </span>
              </div>
            ))}
        ...
        <dl className="grid grid-cols-2 gap-x-4 gap-y-3 border-t border-border pt-3">
          <Stat label="Starting From" value={formatBdt(project.startingPrice)} isHighlight />
